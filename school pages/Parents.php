...
if (isset($_POST['btnsave'])) {
    ...
    if (empty($father_name) || empty($phone)) {
        ...
        $insert = $conn->prepare("INSERT INTO parents (father_name, mother_name, phone, email, address) VALUES (?, ?, ?, ?, ?)");
        $insert->bind_param("sssss", $father_name, $mother_name, $phone, $email, $address);

        if ($insert->execute()) {
            $success = "Parent <strong>$father_name</strong> saved successfully!";
        } else {
            $error = 'Failed to save parent: ' . $conn->error;
        }
        $insert->close();
    }
}
  ...
  <div class="content-wrapper">
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6"><h1 class="m-0">Parents/Guardians</h1></div>
          <div class="col-sm-6">
              ...
              <form action="" method="post">
                ...
                <div class="card-footer">
                  <button type="submit" class="btn btn-primary" name="btnsave">
                    <i class="fas fa-save mr-1"></i> Save Parent
                  </button>
                  <a href="View_parents.php" class="btn btn-secondary ml-2">
                    <i class="fas fa-list mr-1"></i> View Parents
                  </a>
                </div>
              </form>
            </div>
